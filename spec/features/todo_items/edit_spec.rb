require 'spec_helper'

describe "Editing todo item" do
  let!(:todo_list) {
    TodoList.create title: "Groceries", description: "Grocery List."
  }

  def update_todo_list(options={})
    options[:title] ||= "My New T0d0 1!st t!t1e!"
    options[:description] ||= "My New T0d0 1!st descr!pt!0n!!"

    visit "/todo_lists"
    within "#todo_list_#{todo_list.id}" do
      click_link "Edit"
    end

    fill_in "Title", with: options[:title]
    fill_in "Description", with: options[:description]

    click_button "Update Todo list"
    todo_list.reload
  end


  it "updates a todo list successfully with correct information" do
    update_todo_list

    expect(page).to have_content("Todo list was successfully updated")
    expect(todo_list.title).to eq("My New T0d0 1!st t!t1e!")
    expect(todo_list.description).to eq("My New T0d0 1!st descr!pt!0n!!")
  end

  it "displays an error when the todo list has no title" do
    update_todo_list title: ""
    expect(page).to have_content("error")
  end

  it "displays an error when the todo list has a title with less than 3 characters" do
    update_todo_list title: "No"
    expect(page).to have_content("error")
  end

  it "displays an error when the todo list has no description" do
    update_todo_list description: ""
    expect(page).to have_content("error")
  end

  it "displays an error when the todo list has a description with less than 5 characters" do
    update_todo_list description: "Fail"
    expect(page).to have_content("error")
  end
end
